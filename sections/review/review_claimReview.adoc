...
If the URL of the claim refers to a social media post (Facebook, X (former Twitter), Youtube, Reddit, Instagram, Telegram, Mastodon etc.). The type used is SocialMediaPost.
...
| isPartOf | {type_creative_work} or {type_url}| Item that this post is part of. It can be social media account or channel
...
| keywords | {type_text}| keywords or hashtag attached to this post. Multiple textual entries in a keywords list are typically delimited by commas, or by repeating the property. 
|===

NOTE: *creator* or *isPartOf*. Both attributes are valid. However to define the post social account owner and to stay on Schema.org definition, *isPartOf* would be preferred. If creator is used, it will required to used some custom properties as define below.

include::appearence/creator.adoc[]

===== CreativeWork examples
...
| interactionType | https://schema.org/InteractAction[InteractAction]| The act of interacting with another person or organization.
Used action type:
...
==== Appearance specialized content
